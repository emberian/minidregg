...
| Semantic history | implemented semantic fold, consumer-gated | `SemanticHistoryAccumulator` consumes the same manifest and resolved controller registry and retains proof-relevant turns/fold traces | Registry closure is not clause evidence. Each statement/proof-root pair needs a `ClauseEvidenceFamily` adapter; deployed header hashing, code membership, PCS/BCS, ROM/binding, proximity, recursion, and privacy remain separate named boundaries |
