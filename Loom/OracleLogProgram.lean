/-
# Loom.OracleLogProgram — [ORACLE-LOG-program]: the audit finding (the stated
per-slot bound is REFUTABLE at every useful price) and the repair (link-root
opening re-attaches the fixed pair).

`Loom/OracleLogExtraction.lean` sharpened the deployed-ZK adaptive-increment
...
prove it by rerunning `Loom/Depth.lean`'s game-slot choreography.
...
-/
import Loom.OracleLogExtraction
