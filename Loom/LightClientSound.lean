/-
# Loom.LightClientSound — [LC-sound]: the one-shot probabilistic light client.

Closes the quantifier gap `Loom/LightClient.lean` proved load-bearing: the v0
...
* **`lightClientSound` — THE DEPLOYABLE HEADLINE, `n·(err⋆(δ) + 1/|F|)`.**
  ...
  `[ACC-extract]`'s obligation, exactly as in `Loom/AccSound.lean`'s "strict
  ...
  the chain composes by the RBR pattern of `Loom/Depth.lean`'s Thm B.4 union
...
(`[LC-sound-fs]`, riding `Loom/FiatShamir.lean` + `[ACC-sound-rbr]`), and
...
`Fin ch.length → F` — where the theorems of `Loom/LightClient.lean` quantify
...
/-! ## Counting plumbing (private — `Loom/Depth.lean` owns the public
`uniformProb` toolkit, REUSED below; these two facts are the only ones it
lacks, kept `private` by the `Loom/AccSound.lean` discipline so the aggregate
...
`Loom/LightClient.lean`), so it survives one sampled schedule with probability
at most `1/|F|`. -/
...
`ε_rbr` (`Loom/Depth.lean`); the bound here has the same per-move shape. -/
...
* **[LC-sound-fs]** — this file samples the schedule UNIFORMLY (`uniformProb`
  ...
  RBR price" is `Loom/FiatShamir.lean`'s compilation (FS-of-RBR ⟺ [OB-2′],
...
* **[ACC-extract]** (unchanged, `Loom/Accumulator.lean`) — both theorems here
  ...
  `OB2_depth_composition_nonneg_proved` (`Loom/Depth.lean`, PROVED) through
  the same `[ACC-sound-rbr]` packaging.
* **[ACC-sound-list]** (unchanged, `Loom/AccSound.lean`) — beyond unique
