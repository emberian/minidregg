...
named four parts. Part (i)'s UD case landed in `Loom/ProximityGapUD.lean`.
...
* **(ii) — PROVED IN THE LITERATURE FOR THE RS/POLYNOMIAL-GENERATOR
  ...
  shape `Loom/SubUdSeam.lean`'s family ladder consumes — and the STRICT
...
honest hypothesis exactly as `Loom/ReedSolomon.lean` carries it) yields
...
omit [DecidableEq ι] in
/-- **The seam consumer, cited concretely**: `Loom/SubUdSeam.lean`'s
...
* **(a) The Johnson-regime proximity gap** `hPG` at `B = √ρ` (BCIKS
  ...
  (`Loom/ProximityGapUD.lean`, radius `(1 − ρ)/3`, residual
  `[PROXGAP-tight]`).
* **(b) List-member selection** past `dC/2`: `johnson_list_bound` caps the
  ...
  seam (`Loom/OutOfDomain.lean`), untouched here.
