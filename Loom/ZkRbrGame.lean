/-
# Loom.ZkRbrGame — the full ZK-argument RBR game, ASSEMBLED.

`[ACC-sound-rbr-game]` (`Loom/AccSoundRbr.lean`) owed the game object whose
`extract` field the naive backward extractor could not fill;
`Loom/ZkExtraction.lean` FIXED the extract field (the extractor outputs the
...
columns) and `Loom/RbrZeroKnowledge.lean` supplied the simulator vocabulary.
...
* **priced on Loom's proven per-round scale** (`round_error_prox` — the
...
`loom_zk_argument` is the headline: Loom has a straightline non-interactive
...
* **the cryptographic floor** — `[FS-ROM]` (the deployed sponge realizes the
  lazy-sampling handler; `Loom/FiatShamir.lean`), `[COMMIT-CR]` (the deployed
  Merkle scheme realizes `BindingCommitment`; `Loom/Commitment.lean`), and
  ...
  `Loom/ReedSolomon.lean` — consumed as a hypothesis, never claimed);
* **the sub-UD seam** — `[ZK-RBR-extract]` open lemma A (`Loom/ZkExtraction.lean`):
...
universes (`Loom/ZkExtraction.lean`, `Loom/AccSoundRbr.lean`), so `accFsSound`
...
* `completeness` ← `masked_aggregate_satisfies` (`Loom/ZkExtraction.lean`);
* `knowledge_sound` ← `extractChain_sound` (`Loom/AccExtractChain.lean` —
  `[ACC-sound-rbr-game]`'s named chain-level realizer);
* `masked_exact` ← `extractChain_flatFold` (`Loom/ZkExtraction.lean`);
* `pair_extraction` ← `maskedChainExtraction_holds` /
  `extractMaskedPair_sound` (`Loom/ZkExtraction.lean` — the fixed extract
  field);
* `straightline_seam` ← `extractChain_committed_seam`
  (`Loom/ZkExtraction.lean` — `[ACC-extract-bind]` composed end to end);
* `zero_knowledge` ← `rbrZeroKnowledge_of_rounds`
  (`Loom/RbrZeroKnowledge.lean`);
* `round_error_prox` ← `accSound_rbr` (`Loom/AccSoundRbr.lean`);
* `round_error_exact` ← `accSound_rbr_exact` (`Loom/AccSoundRbr.lean`);
* `fs_composed` ← `accFsSound` (`Loom/AccSoundRbr.lean`, riding
  `fsKeystone_proved`, `Loom/FiatShamir.lean`, PROVED unconditionally).
...
/-! ## `loom_zk_argument` — the headline -/

/-- **The ZK completion statement.** Loom has a straightline non-interactive
...
theorem loom_zk_argument [Fintype F] [DecidableEq F] [Fintype ι]
...
the game's `fs_composed` field compiles `Loom/Depth.lean`'s
...
What `Loom/ZkExtraction.lean` REMOVED from this debt — the parts
...
* **the per-round `extract_sound` refinement** — WARP Def 4.2's event
  ...
  falseness-hypothesis choreography `Loom/AccSoundRbr.lean`'s audit
...
* `loom_zk_argument` — PROVED: the headline — every aligned chain with
...
`#print axioms` on `zkRbrGame_holds`, `loom_zk_argument`,
