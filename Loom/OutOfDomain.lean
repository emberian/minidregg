/-
# Loom.OutOfDomain — out-of-domain sampling (WHIR eprint 2024/1586 Lemma 4.25 /
...
`card_agreeSet_lt_of_ne` that gave `Loom/ReedSolomon.lean` its minimum distance,
...
* **The constraint-channel bridge** — `oodConstraint`: the OOD evaluation claim
  `oodEval f z = σ` IS a `LinearConstraint` of `Loom/ConstrainedCode.lean`
...
mutual-correlated-agreement machinery of `Loom/CorrelatedAgreement.lean` (the
...
`Loom/ReedSolomon.lean`, root counting) caps agreement of a DISTINCT pair
strictly below `d ≤ |ι|`. -/
...
WHOLE field: `Loom/ReedSolomon.lean`'s agreement-count theorem at the identity
embedding. -/
...
`uniformProb` (Loom/Rbr.lean, counting — no measure theory) over the OOD
...
congruence (`Loom/Depth.lean` carries the same fact; restated privately here so
this file depends only on `Loom/Rbr.lean`'s `uniformProb`). -/
...
`Loom/ConstrainedCode.lean` promised (via `exists_dotWt_eq`) that OOD
