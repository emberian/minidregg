/-
# Loom.FiatShamir — the transcript layer: the ROM as an inhabited handler, and
...
top of Loom/Rbr.lean and Loom/Depth.lean, statement-first, with built
witnesses.
...
* `srHandlerRun` + the correspondence theorems — the landed SR game of
  Loom/Rbr.lean (`srTrace`, specialization 5's lazy sampling) IS a run of this
...
transcript prefix through round `i` (`SrOutput.query`, Loom/Rbr.lean — exactly
...
Loom/Rbr.lean's state-restoration game lazily samples its oracle via a query
...
recorded responses stay in lockstep with `stepFn` (Loom/Depth.lean). -/
...
`find?` view (which the game uses for consistency, Loom/Rbr.lean
...
[OB-2′] nonnegativity guard: Loom/Depth.lean machine-checked the unguarded
...
[OB-2′] (`OB2_depth_composition_nonneg`, Loom/Depth.lean): the transcript
...
/-- Premise-inhabitation, unconditionally: Loom/Depth.lean's trivial
