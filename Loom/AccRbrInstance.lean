/-
# Loom.AccRbrInstance — [ZK-RBR-game-resid]: the WARP-Def-4.2-NATIVE instance
for the accumulator chain, ASSEMBLED — `fs_composed` fires on the
accumulator's OWN instance.

`Loom/ZkRbrGame.lean` closed the ZK-argument game packaging and named its one
...
rather than on a caller-supplied Def-4.2 instance. `Loom/ZkExtraction.lean`
...
* `accRbrKnowledgeSound` — the Def-4.2 instance, `extract_sound` PROVED: the
  ...
  `Loom/AccSoundRbr.lean`'s audit prescribed). The per-round `∃ w`-event
...
`aggregate`/`foldWords` (`Loom/LightClient.lean`) recurse head-first with a
...
(`Loom/LightClientSound.lean`, the sampled-schedule discipline). -/
variable {Root : Type*} {F : Type} [Field F] {ι : Type*} {r : ℕ}
variable (foldRoot : Root → F → Root → Root)
...
sampled schedule (`Loom/LightClientSound.lean`'s `padSched`). -/
...
/-- `Loom/Rbr.lean`'s `fracHamming` and `Loom/CorrelatedAgreement.lean`'s
...
functionals at every depth (`aggregate_weights`, `Loom/LightClient.lean`),
...
`Loom/Rbr.lean`): `k = ch.length` rounds — one per fold step — with challenge
...
exactly the canonical-radius resolution `Loom/AccSoundRbr.lean`'s audit
...
`Loom/Rbr.lean`'s `KStateFn`): the classical bit of `AccStateProp`. The three
...
`Loom/Rbr.lean`'s `RbrKnowledgeSoundness`) — `[ZK-RBR-game-resid]`'s owed
...
(`Loom/AccSoundRbr.lean`'s proved round price, the SAME function — no slack
...
Construction B.5 (`Loom/Depth.lean`'s `wAt`, `w_k := w'` seed, one
...
(`Loom/AccSoundRbr.lean`, riding `fsKeystone_proved`'s unconditional
...
sentence `loom_zk_argument`'s `fs_composed` field owed. Scope: the IOR-level
...
algebra for every piece is landed in `Loom/ZkExtraction.lean` and
`Loom/Commitment.lean`; the packaging (this file's construction re-run at
