/-
# Loom.ProximityGapUD — the RS proximity gap PROVED in the unique-decoding
...
`Loom/ReedSolomon.lean` (Cor 4.11), `Loom/Proximity.lean`
(`foldDistancePreserving_of_isProximityGenerator`), and `Loom/SubUdSeam.lean`
...
* `reedSolomonCode_isProximityGenerator_UD` — the standing `hPG` of
  `Loom/ReedSolomon.lean` (Cor 4.11 hypothesis), PROVED at
  `B = (2 + ρ)/3`, `err = n/|F|`.
...
* `foldDistancePreserving_UD` — `[PROX-fold-distance]` at macroscopic δ
  (`Loom/Proximity.lean`'s reduction fed the realizer): one FRI fold round
...
`Loom/Sumcheck.lean`'s `uniformProb` — that import chain is lane-blocked;
...
macroscopic-δ regime of `Loom/Proximity.lean`, previously REDUCED to `hPG`
...
* **(d) The Johnson regime** (`B = √ρ`, WHIR Thm 4.8 / BCIKS Thm 1.2
  ...
  `Loom/ReedSolomon.lean`'s keystone TODO and `[SUBUD-johnson]`(i)'s hard
  half.
