/-
# Loom.CollisionResistanceROM — collision resistance IS a ROM theorem: the
# birthday-bound adversary, and the `[COMMIT-CR]` discharge at the flat-hash
# resolution.

`Loom/Commitment.lean` prices the deployed Merkle/sponge realizer as the named
...
`Loom/FiatShamir.lean`'s `Oracle` — collision resistance is not a standalone
...
* **The coin-space model.** A `q`-query adversary with pairwise-DISTINCT
  ...
  `Loom/FiatShamir.lean`'s `Oracle.empty` over the adversary's query list and
...
* **`birthday_bound`** — the core: the probability that two distinct queries
  ...
  (`uniformProb_exists_le`, `Loom/Depth.lean`), each pair colliding with
...
(`splitCoord`, `Loom/Depth.lean`) and condition on everything but its coin
...
queries" — and that modeling step is a THEOREM against `Loom/FiatShamir.lean`'s
...
(`Oracle.empty`, `Loom/FiatShamir.lean`), run on an adversary's `q` distinct
...
`Loom/Commitment.lean`'s vocabulary: for EVERY oracle function `H` consistent
...
(the `srTrace`/`stepFn` pattern of `Loom/Rbr.lean` at query granularity) and
...
`Loom/Commitment.lean`): the Merkle/sponge structure — tree-shaped `commit`,
...
* `roScheme` / `roScheme_break_iff_collision` / `roBinding` — the RO hash
  ...
  carried structure in `Loom/Commitment.lean`'s `BindingCommitment` shape.
