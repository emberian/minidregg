/-
# Loom.DeciderProximity — `[DEC-proximity]`: the DEPLOYED decider — accept δ-close, reject δ-far.

`Loom/Decider.lean` is the strict side of LOOM §1's promise split: `decider`
...
* The two regimes, EXACTLY as Proximity delivers `[PROX-fold-distance]`:
  ...
  - `deciderProx_sound_of_proximityGenerator` — macroscopic δ ∈ (0, 1−B):
    ...
    2020/654 — the same `hPG` Loom/ReedSolomon.lean carries), consumed per
...
* The opened-word seam (`deciderProx_open_sound`, `deciderProxLDT_open_sound`)
  ...
  (Loom/Commitment.lean) transported to the deployed decider, same
...
strict contract of `Loom/Decider.lean` is recovered on the nose. -/
...
Reed–Solomon hypothesis of `Loom/ReedSolomon.lean`, consumed here per level
...
`Loom/Decider.lean` (and with it `AccClaim.Satisfies`) is exactly what the
...
`Loom/AccExtract.lean`'s ledger); (ii) the query-repetition soundness
...
* `deciderProx_open_sound` / `deciderProxLDT_open_sound` — `[DEC-open]`
  transported (all-positions resolution, as in `Loom/Commitment.lean`).
...
* `#print axioms`: every theorem above — `propext`, `Classical.choice`,
  ...
  classical acceptance-set measurability, exactly as in Loom/Proximity.lean). -/
