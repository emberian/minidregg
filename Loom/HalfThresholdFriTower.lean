/-
# Loom.HalfThresholdFriTower -- one halving, then a fixed-radius FRI tail

`Loom.HalfThresholdFri` closes the first multiplicative FRI round:
...
This is soundness for Loom's whole-word `proximityTest`.  A maliciously
