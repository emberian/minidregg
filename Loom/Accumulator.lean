/-
# Loom.Accumulator — the WARP-shape accumulated claim and its γ-fold.

THE HEART of Loom (docs/LOOM-RECOMPOSITION.md §1): the accumulated object
...
* **Linearity scope — the channel is the POST-REDUCTION interface.** The
  ...
  The sumcheck front lives in `Loom/Sumcheck.lean`; this file fixes the
...
* **Sibling reconciliation.** `Loom/ConstrainedCode.lean` (landed
...
(LOOM §1, [OB-3 restated]). (Namespaced: the sibling `Loom/ConstrainedCode`
...
the sibling's Horner-spine `batchTarget` (`Loom/ConstrainedCode.lean`,
...
/-! ## Bridge to `Loom/ConstrainedCode.lean` — one algebra, two presentations
...
/-- The `dotWt` pairing of `Loom/ConstrainedCode.lean` as a bilinear map: in
...
`Loom/ReedSolomon.lean`'s `RSExample` (lines over F₅ on 4 points) with the
...
linear code in `Loom/CorrelatedAgreement.lean`
(`hasMutualCorrelatedAgreement_of_isProximityGenerator`, WHIR Lemma 4.10) and
instantiated for RS in `Loom/ReedSolomon.lean`; the EXACT-WORD kernel of the
same-word case is already a theorem — `card_batch_satisfying_le` in
`Loom/ConstrainedCode.lean` (the defect-polynomial root count, `≤ t−1`
...
`ε_sr ≤ (t+k)·ε_rbr` (WARP Thm B.4) — the tower PROVED in `Loom/Depth.lean`
