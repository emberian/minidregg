/-
# Loom.Decider — D_ACC: the light client's ONE-TIME final check.
...
* `decider_sound` / `decider_complete` / `decider_rejects` — the interface
  ...
  relation every fold-soundness bound of `Loom/AccSound.lean` bounds, so the
...
* `decider_constrainedRS` — on a transported constraint list the decider
  ...
  `CRS[F, dom, d, cs]` (`Loom/ConstrainedCode.lean`): the code membership the
...
`Loom/ConstrainedCode.lean` — the code membership the decider checks is the
...
`Loom/Accumulator.lean`'s `AccExample`. The new ingredient making the decider
...
fold-soundness bounds (`Loom/AccSound.lean`), never in the claim relation.
