/-
# Loom.SpongeIndiffCapacityBudget — exact per-step adaptive avoid sets

The run invariant in `Loom.SpongeIndiffRunInvariant` records the precise
...
needed by `Loom.AdaptiveFiniteSampling`.
...
-/
import Loom.SpongeIndiffRunInvariant
import Loom.AdaptiveFiniteSampling
...
/-- info: 'Minidregg.Loom.idealStep_sim_log_length_le' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in #print axioms idealStep_sim_log_length_le
/-- info: 'Minidregg.Loom.idealCapacityAvoid_card_le' depends on axioms: [propext, Classical.choice, Quot.sound] -/
