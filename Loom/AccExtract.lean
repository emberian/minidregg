/-
# Loom.AccExtract — [ACC-extract] advanced: KNOWLEDGE soundness of the γ-fold.

`Loom/AccSound.lean` and `Loom/LightClientSound.lean` bound the probability
...
* `extractPair_peel₂` — the peel iterated backwards through a depth-2 fold:
  ...
  the constituents — `Loom/Depth.lean`'s Construction B.5 recursion (`wAt`:
...
* **Straightline means no rewinding, and that is all it means here.** The
  ...
  the same discipline as `Loom/Depth.lean`'s `srExtract` (reads the output
  statement, messages, and final challenges; the game trace argument is
  unused) and `Loom/LightClientSound.lean`'s schedule-pair extractor. How a
...
* **The word is the knowledge object; the root is not.** Everything here
  ...
  (`Loom/Accumulator.lean`'s module header).
...
(`Loom/Depth.lean`, WARP Thm B.4 repaired): cited here so the import is
...
`Loom/Accumulator.lean`'s `AccExample` vocabulary (`xWord`, `oneWord`,
...
folds — the honest accumulator × link pair of `Loom/Accumulator.lean`'s
...
form is a single recursion over `Loom/LightClientSound.lean`'s `Chain`:
...
is already PROVED: `OB2_depth_composition_nonneg_proved` (`Loom/Depth.lean`,
...
between this file's algebra and the commitment layer, dual to `Loom/Decider`'s
...
`Loom/CorrelatedAgreement.lean` — the erasure step rides it), which is also
