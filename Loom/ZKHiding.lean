/-
# Loom.ZKHiding — advancing [OB-4-hiding]: the opening-leakage layer's
provable core — MDS t-wise independence and masked-opening hiding.

`Loom/ZK.lean` proved the folded-WORD hiding shell and confined the genuinely
...
straightline accumulator. This refines `Loom/ZK.lean`'s `[OB-4-hiding]`: the
...
* **Hiding and soundness in one game.** `mask_mem` keeps the masked word in
  ...
  RBR soundness bounds (`Loom/AccSound.lean`) and the extractor
