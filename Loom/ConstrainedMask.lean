/-
# Loom.ConstrainedMask — gap (1) of `[OB-4-hiding-rbr]`: constrained-mask
surjectivity, `r = 1`.

`Loom/ZKHiding.lean` proved masked-opening hiding for a mask ranging over the
...
satisfy the mask claim's constraint channel (`Loom/ZK.lean`'s `mask_bijOn`
...
proof (`Loom/ZKHiding.lean`) run on the SCALED targets through the
