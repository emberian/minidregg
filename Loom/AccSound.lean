/-
# Loom.AccSound — [ACC-sound]: the γ-fold's soundness bound.

Closes the probability half of `Loom/Accumulator.lean`'s `[ACC-sound]` residual:
...
* the EXACT-WORD kernel is `card_batch_satisfying_le`
  (`Loom/ConstrainedCode.lean` — the defect-polynomial root count);
* the δ-proximity realizer is mutual correlated agreement at unique decoding
  (`Loom/CorrelatedAgreement.lean` — WHIR Lemma 4.10, proved for every linear
  code) plus its unique-decoding mechanism `codeword_eq_of_close_of_close`;
* the claim algebra is `Loom/Accumulator.lean`'s (`AccClaim.batch`,
  `foldClaims`).
...
* **`uniformProb` comes from `Loom/Rbr.lean`** — one import beyond this lane's
  ...
  stable, already an aggregate import of `Loom.lean`); a local twin under this
  namespace would collide or duplicate (house law: nothing lands beside what
  it supersedes). `Loom/OutOfDomain.lean` is the precedent — it imports Rbr
  for `uniformProb` alone and keeps its counting lemmas `private` to avoid
  clashing with `Loom/Depth.lean`'s toolkit (NOT imported here: sibling lanes
...
* **[ACC-sound-rbr]** — packaging these per-step challenge bounds as the
  round function of the RBR game (`Loom/Rbr.lean`'s vocabulary) so the
  FS/BCS compilation (`Loom/FiatShamir.lean`) consumes them; the bounds here
...
* The `IsProximityGenerator` hypothesis for RS at concrete parameters is WHIR
  Thm 4.8 — a named hypothesis in this tree by the `Loom/ReedSolomon.lean`
...
aggregate `Loom.lean` build never sees two copies). -/
...
mutual-CA error bound of `Loom/CorrelatedAgreement.lean` and the challenge
...
theorem AccClaim.batch_sound_exact [Fintype F] {C : Submodule F (ι → F)}
  ...
  have hbatch : ∀ γ : F, Minidregg.Loom.Satisfies fw (batchConstraint γ cs) ↔
    ...
    have htgsum : batchTarget γ cs = ∑ i : Fin r, γ ^ (i : ℕ) * A.targets i := by
      ...
      rw [htg k]
      rfl
    rw [Minidregg.Loom.Satisfies, batchConstraint, hwtsum, htgsum,
  ...
  have hsub : ∀ γ : F, AccClaim.Satisfies C (A.batch γ) fw →
      Minidregg.Loom.Satisfies fw (batchConstraint γ cs) := by
    rintro γ ⟨-, hc⟩
    have h0 := hc 0
    rw [hbatch]
    exact h0
  -- the violation transports to the list
  have hviol : ∃ c ∈ cs, ¬Minidregg.Loom.Satisfies fw c := by
    obtain ⟨i, hi⟩ := h
    refine ⟨cs[(Fin.cast hlen.symm i : ℕ)], List.getElem_mem _, ?_⟩
    rw [Minidregg.Loom.Satisfies, hwt (Fin.cast hlen.symm i),
...
(`codeword_eq_of_close_of_close` — `Loom/CorrelatedAgreement.lean`'s
...
tree carries as a named hypothesis, `Loom/ReedSolomon.lean` precedent). The
