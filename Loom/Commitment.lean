/-
# Loom.Commitment — the vector-commitment (Merkle/BCS) binding layer.

Everything upstream of this file is word-level algebra. `Loom/Accumulator.lean`
keeps `Root` opaque by design ("that `rt` binds the word is the commitment
layer's obligation"); `Loom/AccExtract.lean` recovers witnesses from the folded
WORDS, with the words' binding to the prover's roots the named seam
`[ACC-extract-bind]`; `Loom/Decider.lean` consumes the witness word `f` as
...
of the hash. Following the discipline of `Loom/FiatShamir.lean`'s ROM (an
...
* **Resolution: all positions opened.** The seam theorems consume openings at
  ...
  agreement PROVED at unique decoding in `Loom/CorrelatedAgreement.lean`).
  That partial-opening lift is obligation (b) of `[ACC-extract-bind]`'s prose
  ledger in `Loom/AccExtract.lean` and stays THERE; this file closes
...
* **Why the fold algebra never inspected `Root`.** `committed_fold_bind`
  ...
  layer. The extraction side (`Loom/AccExtract.lean`'s `extractPair`, which
  ...
  import-light under it — `Loom/Depth.lean` is a sibling-lane surface.)
...
committed word. Word-level extraction (`Loom/AccExtract.lean`: witnesses
...
which is the contract `Loom/Decider.lean` stated itself conditional on. -/
...
`[ACC-extract-bind]`(b), owed in `Loom/AccExtract.lean`'s ledger, riding
`Loom/CorrelatedAgreement.lean` — not re-owed here.
