/-
# Loom.AccExtractChain — [ACC-extract-chain]: knowledge soundness over the CHAIN.

`Loom/AccExtract.lean` proved the single-fold knowledge core: `extractPair`
...
the backwards peel at depth 2. `Loom/LightClientSound.lean` proved the ∃-form
...
* `extractChain` — the chain extractor: from the prover's words at the flat
  ...
  `Loom/AccExtract.lean`), the LIST of per-link witnesses — each entry one
  `extractPair`.
...
* `extract_head_recurses` — the backwards recursion step made a theorem:
  ...
  `wAt` shape of `Loom/Depth.lean`'s Construction B.5 ("each level's extractor
...
* **Where the `n + 1` transcripts come from.** Everything here consumes folded
  ...
  UNCHANGED from `Loom/AccExtract.lean`, and it is exactly why this file's
  honest label is knowledge soundness AT THE WORD LEVEL. The base transcript's
  schedule is FS-derived (`Loom/LightClientFS.lean`); the perturbed schedules
...
* **The failure probability is the landed one.** Extraction here is EXACT
  ...
  `OB2_depth_composition_nonneg_proved` (`Loom/Depth.lean`, PROVED — cited
...
(`aggregate_targets`, `Loom/LightClientSound.lean`) lives there, matching the
...
nothing — `Loom/Accumulator.lean`'s module header). -/
...
schedule. `Loom/Depth.lean`'s `wAt` recursion ("each level's extractor
...
`Loom/LightClientSound.lean` bound, unchanged
...
(`Loom/Depth.lean`, WARP Thm B.4 repaired, `ε_sr ≤ (t+k)·ε_rbr`, loss-free,
...
* **teeth, information-theoretic** — `chain_single_schedule_ambiguous`: two
  ...
  (`Loom/AccExtract.lean`'s `single_fold_ambiguous`, lifted to the chain). -/
...
residual named in `Loom/AccExtract.lean` asked for the `extractPair`-built
...
* **[ACC-extract-bind]** (`Loom/AccExtract.lean`, unchanged) — everything
  ...
  unique decoding in `Loom/CorrelatedAgreement.lean`), and the words must be
...
* **[ACC-sound-rbr]** (`Loom/AccSound.lean`, unchanged) — packaging the
  per-fold bounds and this extractor as an `RbrKnowledgeSoundness` round
  function, so that the SR-game depth composition
  (`OB2_depth_composition_nonneg_proved`, `Loom/Depth.lean` — cited above,
