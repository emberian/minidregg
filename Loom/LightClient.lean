/-
# Loom.LightClient — the whole-history aggregation object and its attestation.
...
is a chain of links; each link carries an `AccClaim` (`Loom/Accumulator.lean`)
...
* **Residual `[LC-sound]`** (prose, end of file): the straightline SOUNDNESS —
  ...
  `Loom/Depth.lean` modulo its one named seam `[OB-2a]`. NOT imported here
...
/-- **A history link**: one step's accumulated claim (`Loom/Accumulator.lean`)
...
/- The prover's recommitment schedule, as in `Loom/Accumulator.lean`: opaque —
...
`Loom/ReedSolomon.lean`'s `xWord` and `Loom/Accumulator.lean`'s `oneWord` and
...
* per fold, a false link survives only on the bad-challenge set —
  ...
  unique decoding in `Loom/CorrelatedAgreement.lean`); the cancellation event
...
* against a forging PROVER, satisfiability is replaced by extraction: WARP's
  ...
  `ε_sr ≤ (t+k)·ε_rbr` — the tower PROVED in `Loom/Depth.lean` modulo its one
  named seam `[OB-2a]` (the lazy-`rnd` game-slot resolver), with Fiat–Shamir
  of RBR preserving straightline extraction (`Loom/FiatShamir.lean`, modulo
  the same seam);
