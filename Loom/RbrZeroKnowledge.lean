/-
# Loom.RbrZeroKnowledge — [ZK-RBR-game] attacked: the round-level
zero-knowledge vocabulary, and the multi-round extract/simulate coexistence.

`Loom/ZkArgument.lean` closed `[OB-4-hiding-rbr]` gap (2) at ONE round and
named `[ZK-RBR-game]` with two missing pieces: (1) **no round-level
zero-knowledge vocabulary exists** — `Loom/Rbr.lean`'s
...
`Loom/AccExtractChain.lean`'s `extractChain`) and simulatable (per-round mask
hiding, `Loom/ZKHiding.lean`/`Loom/ConstrainedMask.lean`), interleaved over
...
* **`RbrZeroKnowledge`** — the missing vocabulary. A round-indexed family:
  ...
  chain extractor and the aggregate consume (`Loom/LightClientSound.lean`'s
...
* **`zkRbrError`** — the round-level ZK error in `Loom/Rbr.lean`'s own
  measure idiom, dual to `Loom/AccSoundRbr.lean`'s `accRbrError`: over ONE
...
the model `Loom/ZKHiding.lean`'s multi-step residual points at ("fresh
...
mirrors `Loom/Rbr.lean`'s one-challenge-alphabet specialization (pad to the
...
`Loom/ZKHiding.lean` (no probability scaffolding; the `uniformProb`-measured
...
the γ-dial of `Loom/ZKHiding.lean` propagated to the chain level. A schedule
...
`Loom/AccSoundRbr.lean` packaged the fold's SOUNDNESS round error as
...
— `Loom/Rbr.lean`'s own `uniformProb F`, the measure
...
* `zero_knowledge` — the round-by-round simulator family at the base
  ...
  `Loom/ZkArgument.lean`, surviving the lift to depth. -/
...
(`Loom/FiatShamir.lean`, unconditional). `[ZK-RBR-interleave]` composes with
...
(`Loom/ZkArgument.lean`), now at the chain. -/
...
* **Missing lemma 1 — triangular joint hiding.** This file's chain
  ...
  `Loom/Accumulator.lean`'s `foldRoot` recommits the folded word each step)
  ...
  schedule invariant of `Loom/ZKHiding.lean`'s multi-step residual (each
...
* **Missing lemma 2 — masked-chain extraction.** `extractChain`'s `n + 1`
  ...
  per round (`[ACC-extract-bind]`, `Loom/AccExtract.lean`;
  `Loom/Erasure.lean`'s `recoverFromColumns` at unique decoding) — the SAME
...
* **Packaging — the game object (both halves).** `[ACC-sound-rbr-game]`
  (`Loom/AccSoundRbr.lean`) still owes the `Reduction`/`KStateFn`/`extract`
