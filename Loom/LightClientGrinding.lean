/-
# Loom.LightClientGrinding — [LC-fs-adaptive]: the grinding adversary pays the
# try-count factor.

`Loom/LightClientFS.lean` closed [LC-sound-fs] at the FIXED-chain level — the
...
`fsKeystone_proved`/`FsOfRbrSound` (`Loom/FiatShamir.lean`, `(t+k)·ε_rbr`,
...
written: `accFsSound_native` (Loom/AccRbrInstance.lean) FS-compiles the
...
`Loom/Depth.lean` owns the public `uniformProb` toolkit (REUSED below:
...
(the `Loom/AccSound.lean` discipline); the injection marginal is this file's
...
`schedCons` discipline of `Loom/LightClientSound.lean`). -/
...
Loom/LightClientFS.lean, is the one-candidate instance of this shape.) -/
...
(`Oracle.lookup_foldl_respond`, Loom/LightClientFS.lean, REUSED; distinctness
...
(`linkPrefix_injective`, Loom/LightClientFS.lean — the load-bearing
...
`(t + k)·ε` per-query shape of `fsKeystone_proved` (Loom/FiatShamir.lean):
...
`(t + k)·ε_rbr` (Loom/FiatShamir.lean, PROVED), here scaling the FIXED-CHAIN
...
(Loom/AccRbrInstance.lean, `(t+k)·accRbrError` on the accumulator's own
...
* **The honest grinding model.** One shared lazy-sampling handler over the
  ...
  `Oracle.lookup_foldl_respond` REUSED from Loom/LightClientFS.lean), every
...
* **[LC-grinding-native]** — two sharpenings, both supplied by the native
  ...
  `Loom/AccRbrInstance.lean` while this file was written:
  ...
  (Loom/FiatShamir.lean, PROVED, unconditional) fired on the native pair at
  ...
  (Loom/AccRbrInstance.lean: the deployed root-and-columns message
...
* **[FS-ROM]** (inherited, Loom/FiatShamir.lean) — the deployed hash
  ...
  there.
* **[ACC-extract]** (unchanged, Loom/Accumulator.lean) — the δ-events here
