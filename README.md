...
|---|---|---|---|
| **Canonical semantic turn** | **S/A.** `AcceptedCellEffect` retains one exact authorization token, effects digest, and pre-root. Policy address and membership are committed by authority state. `CanonicalTransition` derives one post-state and delta. The canonical resource family derives typed and multi-cell conservation from its installed patch, and an executable two-leg `DeclaredHyperedge` now crosses the exact typed-cell bridge. | Digest reflection/CR is explicit. Physical CAS, WAL recovery, replication, liveness, and consumer migration require deployment refinements. | One inspectable reason for one state change; no hidden call-graph semantics or caller-invented resource balance. |
| **Durable settlement** | **S.** The durable model names root compare-and-swap, nullifiers, exact multi-cell charge, history append, idempotency, and crash outcomes. `DurableDataIntent` additionally binds versioned canonical post bytes and read guards; Hyperdocument publication guards the authority root; and fee-first admission preserves fee/nonce settlement when a body is rejected. `ImplementationRefinement` has a staged/committed/compacting WAL inhabitant. | The handler is still a device MODEL — no fsync, torn record framing, page cache, replication, clock, or liveness. A physical store owes representation/step refinement, deployed codecs, root binding, and tariff. No persistence or availability is inferred. | Retries cannot silently double-spend or split a joint turn in an implementation of the model; stale authority cannot race through the guarded commit path. |
| **Hyperdocuments** | **S/A.** Typed content, links/transclusions, stable ranges, marks/annotations, causal events, accepted content+event publication, exact-head history admission, and atomic two-cell publication exist as Lean carriers and theorems. Offline/concurrent merge preserves every current parent: singleton field sources write; 2+ sources become provenance-bearing `ConflictRecord`. An ancestry-backed accepted merge retains the selected-base realization/root; ambiguous/unavailable bases stay absent. `HyperdocumentInterface` adds versioned first-order query/action negotiation with lawful arguments and exact request-indexed authority. The content, authority, and event-log schemas now have exhibited materializers and cells over the canonical sparse carrier. | The exhibited countability codecs and length roots prove non-vacuity; they are not deployment wire formats or cryptographic roots. No arbitrary unique-LCA claim, CRDT conflict policy, physical persistence/finality, authenticated history query resolver, complete editing grammar, search/GC, or migrated UI. PCS/CR/ROM remain premises at history admission. | Users and agents can keep provenance, negotiate the same typed interface, publish linked changes together, branch offline, and see conflicts instead of losing a writer. |
...
| **History and accumulation** | **S/A, conditional P shape.** The retained history supplies the literal BCS/Fiat–Shamir verifier event. The old universally binding checkpoint carrier is formally impossible at positive height and is no longer a deployment target. The raw replacement retains unequal openings and joins history PCS/MCA, additive proximity, extracted collision, and oracle transport on one coin. Scoped history projections preserve exact roots over finite declared footprints without enumerating an infinite schema. | The executable raw verifier/statement, same-coin transcript transport, collision/ROM prices, MCA/code-exact premises, hiding, and deployment remain. | A later light client can verify a long causal story from a small checkpoint without hiding equivocation behind an impossible global binding premise. |
...
are exact.  These countability-selected codecs close vacuity only.  Production still owes
versioned concrete codecs, root functions, artifact pins, and cryptographic binding.
...
remaining gaps are executable raw proof/security joins, reactive-to-durable lifecycle,
a concrete two-parent Hyperdocument merge, and a user/agent consumer cutover.
