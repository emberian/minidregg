...
| **Accumulation** | Loom proves claim folding, exact/UD soundness, arbitrary-depth extraction, and depth composition. `SemanticReceiptRelation` proves that the clean-sheet pre/post/touched quadratic word is exactly a `ReceiptDelta`; `SemanticHistoryAccumulator` admits commit and reject only with exact controller-resolved clause evidence for every statement/proof root, permits append only from a verified predecessor head, and preserves one folded `AccClaim`. `SemanticHistoryStraightlinePcs` supplies the prefix-root extraction interface. `SemanticAdditiveFriCheckpoint` canonically pads that same word into the additive code, derives the same commitment root, and proves KS-good extraction returns the authoritative history head. | The history head remains proof-relevant and retains its entries; append has an explicit recommitment equation. The PCS/checkpoint modules supply interfaces and separate caller-priced error ledgers, not a concrete PCS, shared Fiat–Shamir/ROM game, Merkle CR, sampled decider, or hiding schedule. This is not yet a WARP/FACS deployment or succinct history object. |
