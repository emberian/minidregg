...
| **Indexed lookup** | `LogupIndexLink` proves decoded Boolean address columns give literal unit-vector incidence and exactly the LogUp* pushforward. Tower256 native code is being reduced to incidence, fraction-tree, interpolation, and round-message arithmetic only. | The former handwritten verifier/receipt adapter was deleted; the remaining protocol wrapper is being deleted after arithmetic extraction. The zerocheck/PCS inherits additive proximity and CR/ROM assumptions; this is indexed evaluation, not yet mutable RAM. |
