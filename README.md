...
| **GF(2) tower and additive FRI** | `Theory/BinaryTower*` proves the tower/fold algebra; `BinaryTowerFanPaarCodec` fixes recursive low/high Fan–Paar coordinates, exact 32-byte little-endian round trips, and zero/one/top-generator coordinate teeth. `Loom/AdditiveProximity`, `AdditiveFriTower`, and `AdditiveFriQuery` close the literal characteristic-two quotient tower, adaptive earliest-deviation cover, authenticated coherent paths, and the UD bound `m·2^(ell−1)/|F| + (1−τ)^q`. `Compiler/AdditiveFriReceiptClause` binds the manifest declaration, reversed/high-first basis order, affine domain and rate schedule, roots-before-challenges, coherent queries, terminal, and exactly that ideal predicate/bound. | Concrete commitment binding/CR, cSHAKE-ROM transport, validation of native arithmetic buffers, an instantiated online controller, and the outer accumulator remain explicit. The generic clause is not an admitted base-V1 runtime clause. Rust retains opaque compute; its four-limb Tower implementation, LCH conventions, and selected hash algorithm are not yet generated or checked against the Lean codec. |
| **Indexed lookup** | `LogupIndexLink` proves decoded Boolean address columns give literal unit-vector incidence and exactly the LogUp* pushforward. `Compiler.Logup256ReceiptClause.indexedTableReceiptClause` derives exact indexed evaluation from that semantic trace. `AuthenticatedColumnLogupBridge` now obtains canonical address linkage, Tower arithmetic, exact root bindings, and the required-roots-before-first-challenge schedule from one accepted `AuthenticatedColumnPlan` terminal statement; a missing required root makes the schedule uninhabitable. `Compiler.BinaryTower256Profile` supplies the proved recursive Fan–Paar codec. Native Tower256 retains arithmetic kernels only. | V1 declares distinct `gf2Tower256Carrier` profile `205`, degree `256`, and codec `21`; clause `404` selects it in a locally extended manifest. The clause remains absent from base V1. PCS semantic soundness, concrete commitment CR/binding, Fiat--Shamir ROM, a real LogUp controller instance, mutable RAM, and Rust four-limb correspondence remain explicit. |
| **Accumulation** | Loom proves claim folding, exact/UD soundness, arbitrary-depth extraction, and depth composition. `SemanticHistoryAccumulator` requires exact controller-resolved clause evidence and preserves one folded `AccClaim`; `SemanticHistoryStraightlinePcs` supplies its prefix-root extraction interface. `SemanticHistoryWARPAdditiveJoin` commits each exact link word before its challenge, commits the literal post-challenge fold, projects the opened stream to the unshifted BCS word schedule, and identifies the terminal fold root with `SemanticAdditiveFriCheckpoint`'s level-zero root. `AuthenticatedColumnHistoryBridge` uses the controller's exact `BindingCommitmentScheme` to turn a terminal attestation's roots, columns, and proof bytes into the literal opened BCS link stream, and proves that stream is exactly the scheduled semantic links and challenges. | The history head remains proof-relevant and retains its entries. Invoking the full `AccRbrBcs` reduction still needs the exact reindexing of the genesis/link `AccClaim`s, plus a concrete collision-resistant commitment/PCS, MCA/ROM reduction, sampled decider, and hiding schedule. The commitment binding law is an explicit field of the shared scheme, not a deployed Merkle theorem. The additive and knowledge ledgers are not yet united in one game, and this is not yet a succinct WARP/FACS deployment. |
...
The lookup carrier mismatch is now corrected at the semantic and artifact-data levels: V1's Lean bundle, JSON,
and generated Rust payload include profile `205` at degree `256` with codec `21`, and the local
lookup clause selects it instead of degree-64 `gf2Carrier`. The Lean codec is the exact recursive
Fan–Paar coordinate system, not an arbitrary enumeration. Clause `404` is still extension-only,
and this does not validate native `Tower256` four-limb representation.
...
`AuthenticatedColumnHistoryBridge` removes one more duplicated interface: the accepted
authenticated-column trace and the WARP-shaped link-opening stream use the same literal commit,
open, and verify relation. The bridge transports the recorded root/value/proof-byte triples into
BCS messages and proves their words and challenges equal the semantic schedule. Collision
resistance/binding, Fiat--Shamir, proximity, and succinct extraction remain explicit obligations;
the bridge does not obtain them from a registry pin or a native kernel.
...
- **Tower arithmetic:** keep the proved additive tower, canonical recursive codec, and optimized
  native arithmetic/transform kernels; add an exact native-byte decoding/recheck boundary, then
  emit the rate-aware FRI/OOD controller from Lean before any binary receipt is admissible again.
