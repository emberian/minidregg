...
premises are explicit here. No deployment field or product range is selected.
-/
...
def installProjectionVersion : List UInt8 :=
  "DREGG.RUNTIME.POLICY-INSTALL.COMPLETE-AUTHORITY/v1".toUTF8.toList
...
def authorizationVersion : List UInt8 :=
  "DREGG.RUNTIME.AUTHORIZATION.COMPLETE-REQUEST-CAPABILITY-USE/v1".toUTF8.toList
...
The target resource and selected policy are distinct coordinates. -/
...
role. The authority page is currently v3, complete state v2 and catalogue root v1;
...
def receiverSemantics (template : FactoryTemplate) : Digest :=
  (Sp800185Cshake256.hash "DREGG.NATIVE.RUNTIME.SEMANTICS/v1".toUTF8.toList
    (encode template)).digest
...
def Profile.source {F : Type} [Field F] (template : FactoryTemplate)
    ...
    (orderNoWrap : PredOrder.NoWrap F orderWidth) : Profile F :=
...
def Profile.compilerProfile {F : Type} [Field F] (profile : Profile F) :
  ...
  .source (receiverSemantics profile.template) profile.fieldIdentity
...
theorem Profile.receiverSemantics_exact {F : Type} [Field F] (profile : Profile F) :
    profile.compilerProfile.descriptor?.map (·.receiverSemantics) =
      some (receiverSemantics profile.template) := rfl
