...
instead used as the computation leg of a release-free typed completion, and
that completion constructs the common canonical `AcceptedCellEffect`.

The constructor below is sealed: successful private computation does not imply
release and its positive path neither constructs nor retains release evidence.
A later disclosure must be a separate authorized effect.  Native buffers
remain opaque candidate data and acquire meaning only through the Lean
admission already proved in `BfvNativeBufferAdmission`.
-/
...
/-- The two non-computation legs needed to accept sealed private work.  The
computation field is deliberately absent: it is constructed below from the
checked BFV batch admission.  There is no output opening, release, or
declassification authority in this boundary. -/
structure CompletionLegs
    (request : authority.declaration.Request)
    (outcome : authority.declaration.ComputationOutcome) where
...
noncomputable def completionOfBatchAdmission
    ...
    authority.declaration.ComputationCompletion request outcome where
  authorization := legs.authorization
  inputIdentity := legs.inputIdentity
  computation := by
    have checked : CheckedPrivateEvidence evidencePortal
        (authority.declaration.computationStatementOf request outcome) := by
      rw [statementExact]
...
theorem completion_every_exact_integer_equation
    {request : authority.declaration.Request}
    {outcome : authority.declaration.ComputationOutcome}
    (completion : authority.declaration.ComputationCompletion request outcome)
