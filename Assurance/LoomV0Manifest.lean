...
/-- **`rs_maskedOpeningHiding`** (`Loom/ZKHiding.lean`) — masked-opening
...
/-- **`not_maskedOpeningHiding_zero`** (`Loom/ZKHiding.lean`) —
...
/-- **`constrainedMask_hiding`** (`Loom/ConstrainedMask.lean`,
...
/-- **`zkArgument_of_hiding`** (`Loom/ZkArgument.lean`) — the single-round
...
/-- **`rbrZk_multiround`** (`Loom/RbrZeroKnowledge.lean`) — the multi-round
...
/-- **`triangularHiding_of_rounds`** (`Loom/ZkTriangular.lean`) — the
...
/-- **`extracted_openings_are_jointOpen`** (`Loom/ZkExtraction.lean`) —
...
/-- **`maskedChainExtraction_holds`** (`Loom/ZkExtraction.lean`) — the
...
/-- **`masked_pair_ambiguous`** (`Loom/ZkExtraction.lean`) —
...
/-- **`zkRbrGame_holds`** (`Loom/ZkRbrGame.lean`) — the ASSEMBLED game: an
...
/-- **`accFsSound_native`** (`Loom/AccRbrInstance.lean`,
...
/-- **`loom_zk_argument`** (`Loom/ZkRbrGame.lean`) — **THE OB-4 HEADLINE**:
Loom has a machine-checked straightline NON-INTERACTIVE ZERO-KNOWLEDGE
...
theorem manifest_loom_zk_argument {Root : Type*} {F : Type} [Field F]
    {ι : Type*} {t r : ℕ}
    [Fintype F] [DecidableEq F] [Fintype ι] [DecidableEq ι] [Nonempty ι] :
    (∀ (C : Submodule F (ι → F)) (foldRoot : Root → F → Root → Root)
        (A₀ : AccClaim Root F ι r) (ch : Chain Root F ι r)
...
/-- **`multiAffine_ext`** (`Loom/MultilinearExtension.lean`) — MLE
...
/-- **`mle_retires_constraint`** (`Loom/MultilinearExtension.lean`,
...
Loom's PROVEN sumcheck retires that violation at `v·d/|F|`. Residual at this
...
the tree). The hiding half is the ZK layer (`loom_zk_argument`, §8) — cited,
...
/-- **`lightClientGrinding_sound`** (`Loom/LightClientGrinding.lean`,
...
/-- **`phantomGrind_beats_fixed_bound`** (`Loom/LightClientGrinding.lean`) —
...
theorem manifest_phantomGrind_beats_fixed_bound
    (d : (i : Fin 2) → Fin (Minidregg.Loom.LCGrindExample.grindChs i).length → ZMod 5) :
    1 / (Fintype.card (ZMod 5) : ℝ)
      < uniformProb (Fin 4 → ZMod 5) (fun c => ∃ i,
          Verifies (reedSolomonCode Minidregg.Loom.RSExample.dom₅ 2)
            (aggregate Minidregg.Loom.LCExample.linRoot
              (padSched (fsSchedule
                (Oracle.ofList Minidregg.Loom.LCGrindExample.grindQs c)
                Minidregg.Loom.LCExample.phantomGenesis
                (Minidregg.Loom.LCGrindExample.grindChs i) (d i)))
              Minidregg.Loom.LCExample.phantomGenesis
              (Minidregg.Loom.LCGrindExample.grindChs i))) :=
  Minidregg.Loom.LCGrindExample.phantomGrind_beats_fixed_bound d
...
`loom_zk_argument` (proved in ZK, §8), this completes the private-witness
...
* **`[OB-4-hiding-rbr]`** — CLOSED. The formerly-open research half of the
  ...
  `zkRbrGame_holds`; the headline is `loom_zk_argument`.
...
the COMPLETE v0 are audited below on the same triple: `loom_zk_argument`
