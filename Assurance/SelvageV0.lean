...
fields are, verbatim, the conclusions of:
  ...
  * `Selvage.lightClientKnowledgeSound` (`Selvage/AccExtractChain.lean`) — KNOWLEDGE,
...
(`Selvage/Decider.lean`'s D_ACC). `loomV0_holds` PROVES the bundle by forwarding
...
* `sound` — `Selvage.lightClientSound`: a chain carrying a δ-far-false CLAIMED
  ...
  at most `n · (err⋆(δ) + 1/|F|)`.
* `knowledge` — `Selvage.lightClientKnowledgeSound`: the chain attests
...
theorem loomV0_holds {Root : Type*} {F : Type} [Field F] {ι : Type*} {r : ℕ}
    ...
    (f : ι → F) :
    SelvageV0Guarantee foldRoot C A₀ ch δ errstar ws f₀ γs γalt h₀ hs S w e f where
...
verifying transcript) exactly as `SelvageV0Guarantee`'s docstring flags. -/
...
γ=1 fold). `SelvageV0Guarantee` is non-vacuous end to end. -/
...
theorem ch₁_loomV0_holds :
    SelvageV0Guarantee foldRoot₁ C₁ A₀₁ ch₁ (1 / 8) (fun _ => (0 : ℝ)) [rt₁]
...
* **NO PROVER; performance UNMEASURED.** Every theorem here is a VERIFIER
  ...
  wall-clock cost.
* **A residual this capstone SURFACES, not inherits**: `SelvageV0Guarantee`'s
