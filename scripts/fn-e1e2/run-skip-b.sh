#!/bin/sh
# Adapt only a private copy of fn's qualified two-Store test harness for
# bounded B empty-page progress before the matching R article.
set -eu
...
if [ "${FN_CUT:-none}" != none ]; then
  echo "B skip scenario requires --cut none" >&2
  exit 2
fi
...
if [ "$actual_client_sha" != "$EXPECTED_CLIENT_SHA256" ]; then
  echo "Mini client image hash changed: $actual_client_sha" >&2
  exit 2
fi
...
patch -s -d "$COPY_DIR" -p0 <"$HERE/two_store_join_skip_b.patch"
cp "$HERE/two_store_join_skip_b.patch" "$COPY_DIR/"
