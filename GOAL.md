...
*Became true.* The six wind-down modules are integrated (`3e28d95`). `Selvage`'s BCS
...
Selvage was built with thinned as the work moved into the newer semantic layers, and that
...
- `437b8f8` — the history opening attempt is retained instead of erased. `Selvage`'s
...
- `d8b49ac` + `716f148` — the retained equivocation now has somewhere to go.
  `openingOf` forgets an executable `CommitmentScheme` to a Selvage `OpeningScheme`
...
landed canonical semantic event, Lean-owned dialect controllers, durable settlement model,
Hyperdocument causal medium, and proof-carrying history.** Lean is the sole source of semantics,
protocol control, and acceptance. Rust is generated DTO/dispatch data or opaque fallible compute;
it has no semantics or refinement status. Accelerators are replaceable implementation choices.

## Current authoritative snapshot

Maturity labels are load-bearing:

- **S** — semantic/formal carrier or theorem;
- **A** — Lean-owned admission/controller over bounded bytes/error;
- **P** — concrete cryptographic reductions and one priced game;
- **D** — artifact/handler/storage integration and consumer cutover; and
- **B** — reproducible matched-workload benchmark evidence.

An older “complete,” “deployed,” “runtime,” or “verifier” entry below is a historical statement
about its then-local theorem/gadget/experiment unless explicitly promoted here. Deleted Rust paths
and gadget satisfiability are not current deployment evidence.

### Landed construction

- **Semantic kernel and authority — S/A:** `AcceptedCellEffect` retains the sole request-indexed
  authority, effects digest, pre-root, patch, and exact footprint.
  `AcceptedCellEffectRequestBinding` (`d138245`) adds family-generic lawful argument addressing and
  exact common `argsDigest` equality without adding a second authority, interpreter, patch, or
  post-state. Digest reflection/CR remains an explicit premise. `CanonicalTransition` derives the
  one post, and flat balanced `DeclaredHyperedge`/`MultiCellHyperedge` carriers publish joint turns.
- **Durable settlement — S:** root CAS, nullifiers, exact multi-cell charge, history append,
  idempotency, and crash outcomes are modeled. Deployment must instantiate physical
  `ImplementationRefinement`, transaction linearization/WAL recovery, replication/failover,
  versioned codecs, and tariffs. No persistence, liveness, or availability is claimed.
- **Hyperdocuments — S/A:** typed content, stable ranges, links/transclusion, marks/annotations,
  accepted content+event effects, atomic two-cell publication, exact causal parents, exact-head
  history admission, and finite post layouts are landed. Concurrent merge retains every current
  parent, writes singleton fields, and stores provenance-bearing `ConflictRecord` for 2+ sources.
  Proof-relevant ancestry can select, reject, or leave ambiguous a base; a selected base is opened
  at its exact realized root and feeds the ordinary multi-cell publication inputs. Ambiguous or
  unavailable bases remain explicitly absent. Physical persistence/finality, richer merge policy,
  PCS/CR/ROM, complete operation grammar, and client cutover remain.
- **Pure private computation — S/A arithmetic:** the authoritative core binds program/relation,
  canonical/native/semantic input identity and bridge, output commitment, resource effects,
...
- **Sealed note-spend core — S/A relation acceptance:** `284f359` admits the exact Lean note-spend
  ...
  validated patch under the sole common authority. `73f0949` adds a byte/error controller whose
  statement is projected only from that accepted core, derives roots before its challenge, and
  treats the native proof payload as inert bytes. Its proof-suite pin is still `0`; the soundness
  laws are premises and no hiding construction, proof of knowledge, CR/ROM/PCS, or native semantic
...
- **Tower256 additive FRI — A:** `c7c05b5` closes deterministic control on the shared exact
  ...
  clause, and admits it on one controller coin. `5500d13` removes the caller-supplied cover: the
  game coin is the actual challenge/query sample, the additive event is literal ideal acceptance,
  and the UD price and false-accept cover are Lean-derived. Remaining **P/D/B** work is exact
  cSHAKE-to-uniform-ROM transport; a raw non-binding Merkle/PCS interface plus the adaptive framed-
  XOF collision/CR reduction that derives position binding; far-word FS/proximity; an executable
  reflected checker; versioned proof codecs/IDs; artifact profile; and end-to-end benchmarks.
- **Tower256 lookup — A, gated:** `a0df3f3` supplies an extension-local clause-404 controller with
  ...
  replies, or plan check failure block; only the existing Lean plan can construct
...
- **Ext6 gate proof — A:** `996b29f` closes deterministic Lean transcript/algebra control around
  ...
  events on one finite coin. `8a9e31b` embeds those disjoint Ext6 classes in the global extensible
  ledger on the same coin and preserves their event probabilities and finite union bound. The
  concrete reductions and sampling-bias price remain **P** residuals; no deployed IVC or 137-bit
...
- **History/knowledge — S/A with conditional P shape:** retained history now reconstructs exact
  ...
  and a four-event bound without independence. `f39ba67` removes the external history false-accept
  proxy: the event is now the literal retained-history Fiat--Shamir verifier/knowledge failure and
  is joined with the concrete additive controller on that same ledger. A concrete same-coin
  `PcsCrRomReduction.classify`, exact common-coin/ROM realization, MCA/code-exact premises,
  hiding/sub-UD, and the underlying PCS/CR/ROM reductions still require instantiation.
- **Artifact/native authority — D for identity, not compute correctness:** the canonical artifact
  ...
  The clause-406 deployment extension adds authenticated work `9102`, BabyBear carrier identity,
  empty request codec `9003`, response codec `9005`, and generated 144-byte/36-canonical-word
  candidate output. JSON and embedded Rust payload agree byte-for-byte. Rust remains opaque and
  fallible; there is no Rust semantics, refinement, FFI proof, or cross-language correctness
  theorem.
- **Deployment join — A/D boundary:** `ComposableDeploymentManifest` derives clauses only from real
  controller entries and separately checks manifest, controller registry, artifact, and native
  catalog. Base V1 has zero clauses. The deployment extension admits clause 406 through real work
  `9102`: generated Rust returns an error or 144 candidate bytes, and Lean alone decodes 36
  canonical BabyBear words and checks the descriptor. This is a narrow byte-backed **D** path, not
  Rust semantics or proof-system security. Clause 404 is gated and absent; BFV 901 is reserved with
  zero suite/controller pins.
- **Native dispatch measurement — B, narrowly:** source `54295c6`, evidence `4d1f290` validates
  direct/generated byte identity for work 9101. Dispatch/direct ratios across lengths 1–16384 were
  `0.985–1.040` on hbox and `0.985–1.025` on persvati. This is empirical dispatch evidence, with no
  performance threshold and no semantic or proof-system claim.
- **Integrated source evidence — build only:** hbox archive
  `E-20260810T003037-13290-hbox-3f74f634e0e1-lake` built the integrated source at `3f74f63`
...
- **Binding-free BCS synthesis — S:** `437b8f8` moves `Selvage`'s `ColsOpen`/`ColsConsistent`
...
- **History collision reduction — S/A:** `d8b49ac`, `716f148`. `openingOf` forgets an
  executable `CommitmentScheme` to a Selvage `OpeningScheme` with `positionBinding_iff`
...
1. Instantiate exact additive cSHAKE-to-uniform-ROM transport and the raw-Merkle adaptive collision
   reduction; instantiate the history same-coin PCS/CR/ROM classifier, MCA/code-exactness, and
   hiding prices.
...
6. Benchmark actual admitted end-to-end user/agent workflows; apply Selvage's kill criteria rather than
   preserving a preferred implementation.

## Superseded pre-controller-wave system map (historical)
...
- **Accumulation — proof-relevant semantic history, no native authority:** Selvage's arbitrary-depth
...
- **Authenticated proof control — one typed spine, first concrete deployment open:**
  ...
  errors cannot accept. Its `BindingCommitmentScheme.toSelvage`
...
## Superseded frontier notes (historical)
...
- 2026-08-09 ★★★★★ **Authenticated columns and dual-root WARP/additive join** ✓ at interface
  ...
  fold-root-after-γ ordering and terminal checkpoint equality. `BindingCommitmentScheme.toSelvage`
...
- 2026-08-09 ★★★★★ **Tower256 indexed lookup → semantic root bundle** ✓:
  `Selvage/LogupIndexLink.lean` proves that canonical decoded address-bit columns are Boolean, their
...
- 2026-08-09 ★★★★★ **Clean-sheet receipt relation → actual Selvage accumulator** ✓:
...
- 2026-08-09 ★★★ **Exact Johnson MCA literature bridge** ✓: `JohnsonMcaBridge` translates Haböck 2025/2110 into Selvage without changing constants: the paper's degree-`≤k` convention means `ρ_H=(d−1)/n`, its finite bad-slope count becomes the affine-generator probability, and radius monotonicity supplies the lower band. The arithmetic theorem proves `m=max(3,d)` covers Selvage's exact `sqrt(d/n)` target, while arbitrary caller errors require an explicit envelope. Honest boundary: the GS/BCIKS factorization, discriminant, Hensel, and line-extraction core remains the named hypothesis `HaboeckTheorem2`; it is neither axiomized nor claimed proved locally.
...
- 2026-08-09 **RETRACTED and deleted committed-accumulator reference:** the former exhaustive
  ...
  removed. Selvage retains the accumulator semantics; the next native path will be bounded compute
...
- 2026-08-08 **Selvage/ProximityGapUDTight [PROXGAP-tight] — full-UD gap built to ONE lemma ✓** (audited, 912843b): the RS proximity gap at the FULL unique-decoding radius (1−ρ)/2 (vs the unconditional (1−ρ)/3), via Berlekamp-Welch-over-F(Z), built END-TO-END. rs_proximityGap_UD_full + hasMutualCorrelatedAgreement_UD_full (conditional on hPS). UNCONDITIONAL core: exists_bw_solution (the interpolation system SOLVED, exact BCIKS degree profile — key find: padding deg_X B by one makes the coeff space exceed constraints by exactly ONE F-dim, so existence is rank-nullity, OBVIATING the rank lemma; the |S|>n budget moves into PS condition (3) where BCIKS spends it, pricing err=n/|F|); bw_quotient_at (BW uniqueness at the FULL 2e+d≤n radius); the Steps 3-5 assembly. Residual = EXACTLY ONE named lemma [PROXGAP-BW-ps] = Polishchuk-Spielman divisibility (BCIKS Lemma 4.4 = Spi95 4.2.18) — literature-TRUE, its formalization (resultants + det-multiplicities + Gauss's lemma) a project of its own. Only the 4 heads conditional; every supporting lemma hypothesis-free. Teeth: band_widens (δ=1/5 macroscopic extension). ATLAS triple. **The full-UD floor reduces to ONE classical lemma.**
- 2026-08-08 **HISTORICAL VERIFIER-GADGET RESULT — not deployed IVC/recursion**
  (`fe9b7fc`): an emittable AIR combined then-local sumcheck, fold-query, Fiat–Shamir, and toy
  Merkle-binding gadgets, with a useful collision-fiber attack tooth. Root absorption, real oracle
  openings, domain/multiplicity wiring, concrete CR/ROM, recursion control, and end-to-end
  deployment were residual. The associated Rust protocol path was subsequently deleted.
- 2026-08-08 ★★★ **Compiler/FiatShamirAir [RECURSE-verifier] FS-binding — the SOUNDNESS piece ✓** (audited, 49d218d): the recursive verifier's challenges are HASHES not choices. fsBindingGadget (challenge_k = hashTerm of the transcript prefix, AirHash absorb-squeeze chain, state via wire-sharing); fsBinding_correct (↔ FsBound, sound+complete); boundVerifier = fsBinding ++ sumcheck-verify ++ FRI-query; boundVerifier_correct (↔ FsBound ∧ sumcheck-accepts ∧ FRI-accepts); boundVerifier_challenges_bound (every accepting assignment carries FS-DERIVED challenges). ★ ATTACK DEMO (the soundness proof): the free-challenge forgery (r₀ forged, downstream consistently re-derived) is ACCEPTED by the unbound sumcheck++FRI but REJECTED by fsBinding alone — attributed to exactly the forged squeeze. That IS why FS-binding makes the verifier sound. emit consumed → 'the FS-bound verifier accepted' is a Selvage statement = a recursive proof. Residuals [RECURSE-merkle-bind] (openings→roots via AirMembership = the LAST piece), [RECURSE-fs-bridge] (Selvage FS id, Assurance), [FS-oracle] floor. ATLAS triple. **Crown jewel ONE piece from the whole self-contained light-client verifier.**
- 2026-08-08 **Prover [PROVER-fri] — the FRI LDT ✓ (verified 83 tests)** (f222549): commit/fold-rounds/query/final-constant around the landed fold. fri_prove (Merkle-commit each level, fold via GpuFold/CPU per Selvage's FoldingTower schedule, per-query openings+paths); fri_verify (verify_open both sides + fold-consistency folded==fold_pair(lo,hi,β,tw) + final CONSTANT check). GENUINE LDT: honest RS codeword (n=256 deg<16) accepts+descends-constant; random rejected at final; cheating-constant-final caught by fold-consistency (Merkle roots bind the descent — both mechanisms load-bearing, separately tamper-tested). fold_pair test-PINNED = fri::fold_once = the formula FriQueryVerifierAir arithmetizes = Selvage fold_sq (prover FRI = AIR gadget = verified fold). Honest deltas: Merkle spot-check = Proximity's un-modeled [DEC-open]-adjacent layer (inherited residual, unpriced); challenges caller-supplied = [PROVER-fs] next. Prover: serialize✓ trace✓ commit✓ sumcheck✓ gate-claim✓ fri-fold✓(GPU) fri-LDT✓; next FS transcript → e2e.
- 2026-08-08 ★★ **Compiler/FriQueryVerifierAir [RECURSE-verifier] — the checks COMPOSE ✓** (audited, 352da7b, choice-free): the FRI query's fold-consistency as Lean-authored AIR. FriQueryAccepts (stated first: ∀k tw_k·twoX_k=1 [witnessed inverse] ∧ next_k=(lo+hi)·half+β(lo−hi)·tw ∧ final=last); friQueryVerifier_correct [propext,Quot.sound]; friFoldVal_eq_div_twoX (the fold IS (lo+hi)/2+β(lo−hi)/(2x) = VERBATIM Selvage.Proximity.fold). **recursiveVerifierPair_correct: sumcheckVerifierGadget ++ friQueryVerifierGadget accepts iff BOTH — the APEX COMPOSITION.** Merkle-open = AirMembership. Teeth: FAKE TWIDDLE with fold algebra adjusted to pass STILL rejected (the inverse mul gate load-bearing); quantified forcing (↔v=4/↔t=6/↔fv=3). Residuals [RECURSE-fri-bridge] (Selvage.fold identification, Assurance) + [RECURSE-verifier] wire-binding (FS challenges + Merkle openings bind the free wires). ATLAS triple. **Crown jewel: the whole light-client verifier's checks exist as ONE appendable emittable AIR — proofs verifying proofs, wire-binding the named remaining mass.**
- 2026-08-08 **RETRACTED GPU EXPERIMENT** (`d65ddb2`): a BabyBear⁴ WGSL fold was
  conformance-tested and measured on one Metal workload. The WGSL implementation, dependencies,
  protocol wrapper, and benchmark were later deleted because they formed a second handwritten
  protocol profile. The numbers are historical kernel evidence only.
- 2026-08-08 ★★ **Compiler/SumcheckVerifierAir [RECURSE-verifier] first gadget ✓ — the CROWN JEWEL starts** (audited, 0bcc3ab, choice-free): the sumcheck VERIFIER as Lean-authored AIR. SumcheckVerifierAccepts (stated FIRST: ∀i g0+g1=chainVal, final=chainVal n; chainVal=Selvage's scChain in values, interp r g0 g1=(1−r)g0+r·g1; d=1 by representation); sumcheckVerifier_correct (systemAccepts ↔ SumcheckVerifierAccepts — the arithmetized verifier accepts iff the sumcheck verifier accepts) [propext,Quot.sound] CHOICE-FREE; interp_eq_of_affine (degree-1 bridge). Header cites the shape match theorem-for-theorem (check1=roundSum_zero, check2=succ, check3=last). **RECURSION APEX BUILT (not prose): emit_accepts_iff_fin consumed on the gadget — 'the verifier accepted' is now an EMITTED Selvage-provable statement → a proof verifies a proof.** Teeth: ∀c accepts↔c=1 (claim FORCED), ∀fv↔fv=1 (final forced), tampered-r₀ rejected with per-check attribution. Residuals [RECURSE-sumcheck-bridge] (Assurance-side systemAccepts↔Selvage.SumcheckAccepts) + [RECURSE-verifier] (FS-bind + FRI-query + Merkle-open=AirMembership → the whole light-client-check-as-AIR). ATLAS triple.
- 2026-08-08 ★★★ **Prover [PROVER-fri-fold] — breadstuffs' fold ADOPTED, THREE-WAY AGREEMENT ✓** (audited, bb1fcf0, 70 tests, tree green 2515): the FLAGSHIP GPU-math adoption. Ext4 (BabyBear⁴ X⁴=11: add/sub/mul/square/halve/base_mul); TWO_ADIC_GENERATOR=440564289 (31^15, order 2^27, = deployed 0x1a427a41); fold_once/fold (β←β² per round). CONFORMANCE: OUR fold THEOREM-EQUAL to Selvage's VERIFIED Proximity.fold on the 16-pt two-adic BabyBear domain (friFolded1_eq/friFolded2_eq via fold_eq_of_mul_eq — division-free uniqueness bridge, since ZMod.inv won't kernel-reduce). CONVENTION FINDING: Selvage + the wgsl AGREE, NO mismatch (1/(2x) at g^j IS ½·g^{-j}; the j+n/2 partner IS f(−x); bitrev = storage layout not math — proved). BONUS ext4Mul_correct (the ext-mul formula IS mult in BabyBear[X]/(X⁴−11), AdjoinRoot). Residual [PROVER-field-ext4-irred] (X⁴−11 irreducibility not kernel-decidable at p≈2^31 — field-ness/|F|=p⁴ pricing unclaimed) + ext-valued fold conformance partial (no computable BabyBear⁴). ATLAS triple. OUR fold = deployed kernel's math = Selvage's verified fold. Next [PROVER-fri-wgsl] = GPU dispatch on ember's Metal + BENCHMARKS.
...
- 2026-08-07 eve **HISTORICAL v0 BUNDLE THEOREM** (`e94ae98`): `loomV0_holds` bundled four
  separately proved ideal guarantees for one formal accumulator and deliberately exposed their
  missing transcript coupling. It was not one concrete end-to-end proof system or deployment.
...
- 2026-08-07 eve ★★★★ **v0 TOWER PROVED — [LC-sound] closed, the probabilistic light client** ★★★★ (8dba14a, RIGOROUSLY AUDITED). lightClientSound: the ∀-schedule apex upgraded to the deployable δ-bound ≤ n·(err⋆(δ)+1/|F|); lightClientSound_exact SHARP at 1/|F| (full n-step induction, not a fallback); reuses ACC-sound's foldClaims_sound_proximity_UD (nothing re-derived); [LC-sound-delta] came out EMPTY. The event is the SAME Verifies∘aggregate the apex bound (anti-weakening); the phantom that broke the naive one-shot form is now caught at all-but-1/|F| schedules, bound ATTAINED at 1/5. ALL ATLAS triple.
  → **HISTORICAL FORMAL SOUNDNESS SLICE:** the uniform-schedule, bound-level composition was
  machine-checked for its ideal interfaces. Fiat–Shamir transport, binding/extraction, concrete
  proximity, prover/controller, and benchmark deployment were not closed; this is not the current
  system's deployed soundness tower.
