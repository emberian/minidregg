...
import Theory.ExtensionBasis  -- [RS-basis] the ring-switching connector (Diamond–Posen 2024/504): the B_κ-indexed K-basis of L (cubeBasis, mathlib finBasisOfFinrankEq reindexed), Definition 2.2's packing as a K-LINEAR EQUIVALENCE (packEquiv/packCube — `.symm` IS Thm 3.5's "reversing Definition 2.2", the emulator's step 3), zero-embedding-overhead as a finrank identity + the element-wise lift's 2^κ overhead factor; tower instantiation [T_{j+d} : T_j] = 2^d so κ = d exactly (deployed Binius shape T₀ ⊂ T₇, deg 128); TEETH — wrong κ admits no cube basis, a repeated coordinate makes packing non-injective (extraction dies), and the element-wise lift is NOT surjective hence refused as a packing map
...
import Theory.ZkmlMatmulSum  -- the foldl->Finset.sum BRIDGE every matmul statement waits on, in TWO NAMED STEPS so the boundary is visible: foldl_add_eq_listSum needs only ASSOCIATIVITY (AddMonoid), listSum_finRange_eq_sum is where the ORDER is discarded (AddCommMonoid -- Finset.sum over Fin k cannot be written without it, so that step has no counterexample, it has a missing typeclass). run_matmul_sum: the ordered fold of TOp.denote IS the contraction over a ring-shaped reading. The load-bearing property is ASSOCIATIVITY, not commutativity: IEEE-754 addition commutes and does not associate, and SatExample REFUTES the bridge on a real trace with a saturating add (denotation 5, Finset.sum 9, kernel-decided) plus an order-reversal tooth. Padding: padded_contraction + padded_matmul_table zero-extend to a DYADIC domain with the contraction unchanged -- so matmul needs NO new TOp constructor and NO non-dyadic MLE machinery (the build log's 4.5 fork resolves to neither branch); the pad is a COMMITMENT-layer obligation, not a denotational one.
